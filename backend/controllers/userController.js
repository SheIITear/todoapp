...
const postLogin = async (req, res, next) => {
    const toolong = 'Invalid credentials';
    try {
        const userFromDb = await selectUserByEmail(req.body.email);
        if (userFromDb.rowCount === 0) return next(new ApiError(toolong, 401));
        
        const user = userFromDb.rows[0];
        if (!await compare(req.body.password, user.password)) return next(new ApiError(toolong, 401));
